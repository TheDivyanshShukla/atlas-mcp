...
async function readLocalFile(repoRoot: string, localPath: string): Promise<string> {
  const abs = resolve(repoRoot, localPath);
  const rel = relative(resolve(repoRoot), abs);
  if (rel.startsWith("..")) throw new Error("localPath must stay inside the repo");
  return readFile(abs, "utf-8");
}

function withDefaultProject(args: Record<string, unknown>, boundRef: string | undefined, toolName: string) {
  if (!boundRef || NO_PROJECT_DEFAULT.has(toolName)) return args;
  if (typeof args.projectId === "string" && args.projectId.trim()) return args;
...
export async function runServer() {
  ...
  if (!boundRef) {
    try {
      const who = await client.get<{ projects: { id: string; name: string }[] }>("/api/v1/whoami", { cache: false });
  ...
  for (const tool of manifest.tools) {
    server.registerTool(
      tool.name,
      { description: tool.description, inputSchema: dynamicInput },
      async (args) => {
        try {
